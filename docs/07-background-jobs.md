...
| Work | For each `LIVE` / `CRIT` / `PRE` game on today's date: upsert boxscore, update state/score/period, invalidate `pulse_narrative:{league}:*` on `LIVE|CRIT → OFF|FINAL` transition |
