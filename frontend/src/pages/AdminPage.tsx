...
const AdminPage = () => {
  ...
  const { leagues, loading: leaguesLoading, createLeague } = useLeagues(user?.id, isSuperAdmin);

  const flashError = (msg: string) => { setErrorMsg(msg); setTimeout(() => setErrorMsg(null), 6000); };
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        subtitle="Create and manage your leagues"
      ...
      <div className="fantasy-card">
        <div className="card-header"><h2 className="text-xl font-bold">Create New League</h2></div>
        <div className="p-6">
          <div className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-3">
            <input type="text" value={newLeagueName} onChange={(e) => setNewLeagueName(e.target.value)} placeholder="League name..." className="w-full px-4 py-2 border-2 border-[#1A1A1A] rounded-none focus:ring-2 focus:ring-[#2563EB]/40 focus:border-[#2563EB] outline-none transition-all" />
            ...
            <button onClick={handleCreateLeague} disabled={creating || !newLeagueName.trim() || !newTeamName.trim()} className="btn-gradient disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap w-full sm:w-auto">{creating ? "Creating..." : "Create & Join"}</button>
          </div>
        </div>
      </div>
      ...
      {/* Your Leagues */}
      <div className="fantasy-card">
        <div className="card-header"><h2 className="text-xl font-bold">{isSuperAdmin ? "All Leagues" : "Your Leagues"}</h2></div>
        <div className="p-6">
          {leaguesLoading ? <LoadingSpinner size="small" message="Loading leagues..." /> : leagues.length === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {leagues.map((league) => {
                ...
                return (
                  ...
                  >
                    <p className="font-extrabold text-[#1A1A1A] uppercase tracking-wider group-hover:text-[#2563EB] transition-colors">{league.name}</p>
